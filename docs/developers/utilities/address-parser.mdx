...
- `parsed.lnurl.type === "pay"` for **LNURL-pay** / Lightning address inputs, can be used as a destination for [Smart Chain → BTC/Lightning](/developers/swaps/smart-chain-to-btc) swaps.
- `parsed.lnurl.type === "withdraw"` for **LNURL-withdraw** inputs, can be used as source for [Lightning → Smart Chain](/developers/swaps/lightning-to-smart-chain) or [Lightning → Solana](/developers/swaps/solana/lightning-to-solana) swaps.
...
If the input is a supported smart chain address, the parser returns the configured chain identifier as the `type`, for example `SOLANA`, `STARKNET` or `CITREA`. This is the format you typically use as the destination in [Bitcoin → Smart Chain](/developers/swaps/btc-to-smart-chain) and [Lightning → Smart Chain](/developers/swaps/lightning-to-smart-chain) swaps.
