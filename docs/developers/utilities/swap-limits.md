...
Like the other route utilities, swap limits are pair-specific and LP-dependent. Use [Supported Tokens](/developers/utilities/supported-tokens) first to choose a valid route, then use `getSwapLimits()` to understand how large or small the input / output amounts can be.
