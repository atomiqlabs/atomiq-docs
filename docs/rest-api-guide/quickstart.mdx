...
[//]: # (TODO: Add reference to the getSwapStatus returned object from rest api reference)

The response is a **swap record** — the same shape every subsequent `GET /getSwapStatus` call will return:
...
Show the `address` and `amount` from each entry of `action.txs` to the user (or use a `hyperlink`, which can be displayed as a QR code, or used as clickable deeplink) and let them pay from whichever wallet they like. Keep polling `getSwapStatus`; the server detects the incoming payment and advances the state automatically.
