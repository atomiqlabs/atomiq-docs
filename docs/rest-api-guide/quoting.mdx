...
:::tip
Call `getSupportedTokens` **once** at startup to populate the source and destination token picker, then call `getSwapCounterTokens` when the user selects either the source or destination token to narrow down the token picker on the other side. You don't need to re-fetch when the user changes the amount.
:::
