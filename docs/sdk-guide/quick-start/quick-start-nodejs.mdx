...
:::warning[Implement swap recovery — refunds & manual claims]
...
- [**Refundable swaps**](/sdk-guide/swap-management/refunds#get-all-refundable-swaps) — failed Smart Chain → BTC swaps where the user's locked funds must be refunded via `swapper.getRefundableSwaps()`.
- [**Claimable swaps**](/sdk-guide/swap-management/claiming#get-all-claimable-swaps) — swaps where the user still needs to claim funds on the destination chain, retrieved via `swapper.getClaimableSwaps()`.
...
Atomiq's [`AbstractSigner`](/sdk-reference/api/atomiq-sdk/src/type-aliases/AbstractSigner) object wraps around the wallet object of the respective chain and makes it easy to approve transactions via it by passing it to the swap methods. You can alternatively skip this step, obtain the transactions required to be signed and [sign & send them manually](/sdk-guide/advanced/manual-transactions).
...
<details>
<summary>Using Solana wallet with private key</summary>
...
<details>
<summary>Using a new Starknet keypair account</summary>

For a programmatic integration driven by a raw private key, use `StarknetKeypairWallet` only for newly created accounts. It derives its address using its own bundled single-key OpenZeppelin account class. If you import a private key from another wallet account (Braavos, Xverse, Argent/Ready, or a different OpenZeppelin version), you will get a **different, empty address**. Your funds still sit at the address derived by the original wallet.
...
const wallet = new StarknetKeypairWallet(starknetProvider, starknetKey);
const starknetSigner = new StarknetSigner(wallet);
```

The account contract does not need to be deployed up front: the SDK automatically deploys it with the first swap transaction (paying the deployment fee from the account's STRK balance).

</details>


<details>
<summary>Using an already-deployed Starknet account</summary>

For a programmatic integration driven by a raw private key controlling an already-deployed account, keep using that account's address: don't pass its private key to `StarknetKeypairWallet`. Use `DeployedStarkCurveWallet` instead. The address is passed explicitly because Starknet addresses depend on the account's class hash.
...
const starknetProvider = new RpcProvider({nodeUrl: starknetRpc});
const wallet = await DeployedStarkCurveWallet.createAndVerify(
  starknetProvider,
  starknetKey,
  "0x..." // your existing, already-deployed account address
);
const starknetSigner = new StarknetSigner(wallet);
```

Prefer `createAndVerify(...)` over using the constructor directly. It checks the account's own on-chain signature validation and throws `AccountNotDeployedError` if the account is not deployed, or `WalletVerificationError` if the key does not control it or the account requires a scheme a single Stark-curve key cannot satisfy, such as multisig.
...
<details>
<summary>Using an EVM wallet with private key</summary>
...
**[Swapper Configuration →](/sdk-guide/advanced/configuration)**
